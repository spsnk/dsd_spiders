...
#include <errno.h>              //Saber si el tiempo de recepción ha transcurrido
...
SocketDatagrama::SocketDatagrama (int pto) {
  ...
  bind (s, (struct sockaddr *) &direccionLocal, sizeof (direccionLocal));       // Asignar Ip y puerto al socket mediante la estructura
}
...
SocketDatagrama::recibe (PaqueteDatagrama & p) {
  ...
  int n =
    recvfrom (s, p.obtieneDatos (), p.obtieneLongitud (), 0,
              (struct sockaddr *) &direccionForanea, (socklen_t *) & clien);
...
SocketDatagrama::envia (PaqueteDatagrama & p) {
  ...
  int n =
    sendto (s, p.obtieneDatos (), p.obtieneLongitud (), 0,
            (struct sockaddr *) &direccionForanea, clien);
  return n;
}
